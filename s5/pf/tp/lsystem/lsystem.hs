...
interpreteSymbole c (e@(p, a) : es, pth : ps) 'F' = (avance c e : es, (fst (avance c e) : pth) : ps)
interpreteSymbole c (e@(p, a) : es, pth : ps) '+' = (tourneAGauche c e : es, (fst (tourneAGauche c e) : pth) : ps)
interpreteSymbole c (e@(p, a) : es, pth : ps) '-' = (tourneADroite c e : es, (fst (tourneADroite c e) : pth) : ps)
interpreteSymbole c (es, ps) '[' = (head es : es, ps)
interpreteSymbole c (_ : es, ps) ']' = (es, ps)
...
interpreteMot c m = Line (head (snd (f m)))
  where
    f :: Mot -> EtatDessin
